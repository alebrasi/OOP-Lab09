...
public final class LambdaUtilities {
    ...
    public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
         ...
         */
        final List<Optional<T>> tmp = new ArrayList<>(list.size() * 2);
        list.forEach(el -> tmp.add(Optional.of(el).filter(pre)));
        return tmp;
    }
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        list.forEach(el -> {
            result.merge(op.apply(el), new HashSet<T>(Set.of(el)), (a, b) -> {
                a.addAll(b);
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((k, v) -> {
            v.ifPresentOrElse((value) -> result.put(k, value), () -> result.put(k, def.get()));
        });
        return result;
    }
